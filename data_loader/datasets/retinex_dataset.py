...
class RetinexNetDataset(BaseDataset):
    def __init__(self, fine_size, training, dataset_name='RetinexNet'):
        super(RetinexNetDataset, self).__init__()
        ...
        for dataset_dir in self.dataset_dir_list:
            for gt_name in os.listdir(self.dataset_base_dir + dataset_dir + self.high_dir):
                if gt_name[-4:] == ".png":
                    self.img_name_list.append(dataset_dir + gt_name)
                    self.low_dataset.append(self.read_img(self.dataset_base_dir + dataset_dir + self.low_dir + gt_name))
                    self.gt_dataset.append(self.read_img(self.dataset_base_dir + dataset_dir + self.high_dir + gt_name))

        self.dataset_size = len(self.gt_dataset)
    ...
    def __getitem__(self, idx):
        if self.training:
            ...
            return to_tensor(low_img), to_tensor(gt_img)
